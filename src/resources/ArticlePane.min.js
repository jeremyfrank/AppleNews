...
Craft.AppleNews.ArticlePane = Garnish.Base.extend({
	...
	handleOptionSelect: function(ev) {
		...
		switch ($a.data('action')) {
			...
			case 'post-article': {
				...
				Craft.postActionRequest('apple-news/article/post-article', data, $.proxy(function(response, textStatus) {
		...
		for (var channelId in newInfos) {
			...
			if (this.hasInfoChanged(channelId, 'state', newInfos)) {
				...
				if ($.inArray(newInfos[channelId]['state'], ['QUEUED', 'QUEUED_UPDATE']) == -1 && !this.versionId && !this.draftId && newInfos[channelId]['canPublish']) {
					$menu.append($('<li><a data-action="post-article">'+Craft.t('apple-news', 'Publish to Apple News')+'</a></li>'));
				} else {
				...
				var downloadUrl = Craft.getActionUrl('apple-news/article/download-article', {
					entryId: this.entryId,
	...
	refresh: function() {
		...
		Craft.postActionRequest('apple-news/article/get-article-info', data, $.proxy(function(response, textStatus) {
			this.$spinner.addClass('hidden');
			if (textStatus == 'success') {
				this.updatePane(response.infos);
			}
