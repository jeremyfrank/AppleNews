...
Craft.AppleNews.ArticlePane = Garnish.Base.extend({
	...
	handleOptionSelect: function(ev) {
		...
		switch ($a.data('action')) {
			...
			case 'post-article': {
				...
				Craft.postActionRequest('apple-news/post-article', data, $.proxy(function(response, textStatus) {
	...
	updatePane: function(newInfos) {
		for (var channelId in newInfos) {
			...
			if (this.hasInfoChanged(channelId, 'state', newInfos)) {
				...
				var downloadUrl = Craft.getActionUrl('apple-news/download-article', {
					entryId: this.entryId,
	...
	refresh: function() {
		...
		Craft.postActionRequest('apple-news/get-article-info', data, $.proxy(function(response, textStatus) {
			this.$spinner.addClass('hidden');
			if (textStatus == 'success') {
				this.updatePane(response.infos);
			}
